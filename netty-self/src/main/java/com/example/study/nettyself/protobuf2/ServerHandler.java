 ...
 */
public class ServerHandler extends SimpleChannelInboundHandler<MyMessagePOJO.MyMessage> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, MyMessagePOJO.MyMessage msg) throws Exception {
        ...
        if (dataType == MyMessagePOJO.MyMessage.DataType.PersonType){
            MyMessagePOJO.Person person = msg.getPerson();
            System.out.println("收到客户端[" + clientIp + "]发送的Person对象：" + person.getId() + "-" + person.getName());
        } else if (dataType == MyMessagePOJO.MyMessage.DataType.WorkerType) {
            MyMessagePOJO.Worker worker = msg.getWorker();
            System.out.println("收到客户端[" + clientIp + "]发送的Worker对象：" + worker.getAge() + "-" + worker.getName());
        } else {
